            ...
            <div className="flex-1 overflow-hidden px-4 py-4">
                <div className="mx-auto flex h-full max-w-6xl overflow-hidden rounded-3xl border border-slate-200/80 bg-white/90 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur">
                    <div className="flex flex-1 flex-col overflow-hidden">
                        ...
                        <div className="flex-1 space-y-5 overflow-hidden p-6">
                            {messages.map((msg) => (
                                <div key={msg.id} className={`flex items-start gap-3 ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
                                    {!msg.isUser && (
                                        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-blue-100 bg-gradient-to-br from-blue-500 to-cyan-400 font-semibold text-white shadow-sm">
                                            AI
                                        </div>
                                    )}
                                    <div
                                        className={`max-w-[70%] rounded-2xl px-4 py-3 leading-7 shadow-sm ${
                                            msg.isUser ? 'rounded-br-md bg-blue-600 text-white' : 'rounded-bl-md bg-slate-100 text-slate-800'
                                        }`}
                                    >
                                        {msg.text}
                                    </div>
                                    {msg.isUser && (
                                        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-slate-300 bg-slate-200 font-semibold text-slate-700 shadow-sm">
                                            我
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="flex flex-none items-center border-t border-slate-100 bg-white/95 p-4">
                            <Input
                                value={inputValue}
